from NOSA_v2 import predict_tumor 
import sys
import numpy as np
import mat73
import os
...
#check if the heatmap file exists in the same directory as the script
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.title_label = QLabel("NOSA v2.0")
        ...
        # Load Image Button
        self.load_img_btn = QPushButton("Load Image (.mat)")
        ...
        self.find_tumors_btn.setStyleSheet("background: #800020; color: white; font-size: 16px;")
        self.find_tumors_btn.setFixedSize(300, 30)
        ...
        self.show_gt_btn = QPushButton("Show Ground Truth Mask")
        self.show_gt_btn.setStyleSheet("background: #16a085; color: white; font-size: 16px;")
        self.show_gt_btn.setFixedSize(300, 30)
        ...
        self.false_positive_toggle = QCheckBox("Apply heatmap to reduce false positives in skull and eye regions")
        self.false_positive_toggle.setChecked(False)
        self.false_positive_toggle.setVisible(False)
        self.false_positive_toggle.stateChanged.connect(self.toggle_confidence_slider)
        self.main_layout.addWidget(self.false_positive_toggle, alignment=Qt.AlignCenter)
        ...
        # Strength slider
        self.confidence_slider_label = QLabel("Strength: 70%")
        self.confidence_slider_label.setAlignment(Qt.AlignCenter)
        ...
        # Create a cover widget to hide the slider and label
        self.slider_cover = QWidget(self)  # Make it a child of MainWindow
        self.slider_cover.setStyleSheet("background-color: #f0f0f0;")  # Match background color
        self.slider_cover.setFixedHeight(70)  # Cover both label and slider

        # Don't add the cover to the layout, it will be positioned manually
    ...
    def update_confidence_label(self, value):
        self.confidence_slider_label.setText(f"Strength: {value}%")
    ...
    def load_file(self):
        ...
        if file_path:
            ...
            if file_path.endswith(".mat"):
                ...
                cjdata = mat['cjdata'] #.mat files have to be structured as in the cheng dataset
    ...
    def find_tumors(self):
        ...
        # 1. Get predicted probabilities (0-1)
    ...
    def show_ground_truth_mask(self):
        if self.ground_truth_mask is not None and self.original_image is not None:
            ...
            QMessageBox.warning(self, "No Ground Truth", "No ground truth mask available for this image.") #error message if no ground truth mask

    def showEvent(self, event):
        #heuristic approach at hiding the strength slider until the checkbox is checked
        super().showEvent(event)
        # Position the cover widget over the slider area
        label_pos = self.confidence_slider_label.pos()
        label_height = self.confidence_slider_label.height()
        slider_height = self.confidence_slider.height()
        
        self.slider_cover.setGeometry(
            0, 
            label_pos.y(), 
            self.width(), 
            label_height + slider_height
        )
        self.slider_cover.raise_()  # Bring the cover to the front

    def resizeEvent(self, event):
        #resizes the cover if the window is resized
        super().resizeEvent(event)
        if hasattr(self, 'slider_cover'):
            label_pos = self.confidence_slider_label.pos()
            label_height = self.confidence_slider_label.height()
            slider_height = self.confidence_slider.height()
    ...
    def toggle_confidence_slider(self):
        is_checked = self.false_positive_toggle.isChecked()
        self.slider_cover.setVisible(not is_checked)  # Hide cover when checkbox is checked
